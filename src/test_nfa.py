...
def test1():
    ...
    test_nfa = NFA.NFA(etats,alphabet,lambda x,y: d[x][y],start,accept)
...
def test2():
    ...
    nfa = NFA.NFA(etats,alphabet,delt,start,accept)
    dfa = nfa.toDFA()
    ...
    #An NFA created from the re
    nfa_from_reg = regexToNFA.regexToNFA('b*(ab*ab*)*|a*(ba*ba*ba*ba*)*')

    #dfa version

    dfa_from_reg = nfa_from_reg.toDFA()
